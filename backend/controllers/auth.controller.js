...
export const signUp = async (req, res) => {
    try {
        ...
        if (mobile.length < 10) {
            return res.status(400).json({ message: "Mobile no must be at least 10 digits long." })
        }
        ...
        return res.status(500).json(`sign up error occur ${error}`)
    }
}
...
export const signIn = async (req, res) => {
    try {
        ...
        return res.status(500).json(`sign in error occur ${error}`)
    }
}
