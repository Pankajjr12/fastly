...
export const placeOrder = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ message: "set complete delivery address" });
    }
...
export const getCurrentOrder = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `not get order error ${error}` });
  }
};
...
export const sendDeliveryOtp = async (req, res) => {
  try {
    ...
    // ❗ Check BEFORE accessing shopOrders
    if (!order) {
      return res.status(400).json({ message: "Order not found" });
    }

    // 2️⃣ FIND SHOP ORDER SAFELY
    const shopOrder = order.shopOrders?.find(
      (s) => s._id.toString() === shopOrderId
    );
